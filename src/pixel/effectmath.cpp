...
namespace EffectMath_PRIVATE {
    ...
    CRGB *getUnsafeLedsArray(){return leds_managePtrGet()->get_crgbArray(0);}
    // CRGB  * leds = getUnsafeLedsArray();
...
void EffectMath::dimAll(uint8_t value) { CRGB * leds = getUnsafeLedsArray();for (uint16_t i = 0; i < NUM_LEDS; i++) {leds[i].nscale8(value); } }
void EffectMath::blur2d(uint8_t val) {CRGB * leds = getUnsafeLedsArray();EffectMath::blur2d(leds,WIDTH,HEIGHT,val);}
